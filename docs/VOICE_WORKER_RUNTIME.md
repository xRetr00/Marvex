...
- Package-specific model adapters now provide the default installed-asset STT/TTS runtime path for Moonshine v2, SenseVoice-Small/FunASR, Supertonic V2, and Piper. These package-specific model adapters resolve assets only through `VoiceAssetManager`, consume captured audio refs from memory, write synthesized PCM only to the in-memory generated-audio sink, and return safe refs/status instead of raw audio or transcript payloads.
