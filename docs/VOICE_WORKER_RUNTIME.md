...
Always-running 24/7 wakeword supervision, real physical microphone validation in CI, real model downloads, default live heavy STT/TTS inference against installed model files without injected runners, package-specific Moonshine/SenseVoice/Kokoro/Piper model invocation, echo suppression, Orb, Face UI, desktop overlay, final visual assistant shell, vision, proactive non-voice behavior, and remote worker exposure remain not implemented unless a later explicit goal widens scope.
