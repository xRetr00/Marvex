...
- Allowed dependencies: Contracts, ProviderPort, approved adapters,
...
- Forbidden dependencies: UI, tools, memory, voice, desktop context, service
  workers, and unapproved runtime behavior.
...
- Allowed dependencies: Contracts, local ProcessRuntime, CLI's approved narrow
  ProcessRuntime integration.
- Forbidden dependencies: HTTP server, daemon, subprocess runtime, service mode,
...
- Forbidden dependencies: provider SDKs, services, HTTP servers, config files,
...
- Forbidden dependencies: adapters, CLI, services, ProviderRuntime,
...
- Allowed dependencies: approved contracts only.
- Forbidden dependencies: Core, adapters, CLI, telemetry implementation,
...
- Allowed dependencies: approved contracts and approved external SDKs inside
  adapter boundaries.
- Forbidden dependencies: Core, ports importing adapters, CLI, telemetry,
...
- Responsibility: Minimal provider creation boundary for approved provider names.
- Allowed dependencies: ProviderPort and approved provider adapters.
- Forbidden dependencies: Core, CLI, telemetry implementation, services,
...
- Responsibility: Build `HealthCheck` and `VersionInfo` from explicit in-memory
  configuration.
- Allowed dependencies: approved contracts and approved standard-library modules.
- Forbidden dependencies: Core, adapters, ProviderRuntime, telemetry, apps,
...
- Responsibility: Minimal trace event construction and sink contracts for the
  turn lifecycle.
- Allowed dependencies: approved contracts.
- Forbidden dependencies: adapters, CLI, services, persistent storage, secrets,
  provider behavior.
...
- Forbidden dependencies: CI-only assumptions, product runtime behavior,
  service workers.
...
- Forbidden dependencies: product runtime behavior, SDK calls, service startup,
  external systems.
