...
- Thin-client shell: `Supervisor::attach` + shared token file (`service_token.rs`); the shell connects to the running service instead of spawning its own backend when the service token is present, else self-supervises (dev).
