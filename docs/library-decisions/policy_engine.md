...
why use it: PyCasbin is a good component candidate for capability and permission decisions once Marvex has policy contracts. Task 034 adopts a minimal injected-enforcer adapter boundary for allow/clarify/deny checks. Pydantic AI and Outlines are useful references or future components for typed structured outputs, but both carry too much framework/runtime gravity for Task 034.
...
scope: Task 034 adopts a thin optional-import PyCasbin adapter boundary only. No broad policy runtime, agent runtime, tool runtime, or structured prompt framework is added.
...
adopt / defer / reject decision: Adopt PyCasbin behind a thin Marvex adapter boundary. Because dependency files are forbidden in Task 034, real package declaration and installation remain deferred to a dependency-governance task. Defer Outlines for structured local constrained generation. Reject Pydantic AI as a central runtime for Marvex now; it may be reconsidered later for isolated typed agent experiments, but not for Core, intent routing, or policy ownership.

risks: PyCasbin policy files can become another hidden authority if not generated from explicit Marvex contracts. Until `pycasbin` is declared in pyproject, real-library construction reports a structured dependency-unavailable error when the package is absent. Pydantic AI can pull Marvex toward a framework-owned agent runtime. Outlines depends on compatible generation backends and may be unnecessary if provider-native structured outputs are sufficient.
