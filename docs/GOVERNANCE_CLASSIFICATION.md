...
| service placeholders | draft service contract | services/* README-only placeholders | README-only until the matching contract is listed in `docs/CONTRACT_APPROVALS.md` and a service-owned entrypoint task exists. Contract approval alone does not authorize implementation. |
