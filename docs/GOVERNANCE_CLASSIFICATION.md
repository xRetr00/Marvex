...
| desktop agent worker | documented surface | services/desktop_agent plus packages.desktop_agent_runtime | Approved local-only perception worker. Reads focused-window content through Windows UI Automation adapters and consumes screenpipe recall through the existing MCP adapter path. Returns safe bounded projections only; raw screen frames, audio, keystrokes, transcripts, MCP payloads, and secrets are not persisted by Marvex. |
| computer-use runtime | policy-controlled surface | services/tool_worker plus packages.adapters.capabilities and packages.desktop_agent_runtime | Approved opt-in execution surface for browser-use and Windows desktop computer-use actions only after CapabilityRuntime policy and approval resume. Browser-use, UFO, OmniParser, and UI Automation stay behind controlled adapters; credential entry, arbitrary desktop control, CAPTCHA bypass, purchase/payment, and raw screen/DOM/action payload persistence remain denied. |
