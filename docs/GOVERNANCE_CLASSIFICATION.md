...
| browser/computer-use adapter/seam | policy-controlled surface | packages.adapters.capabilities.browser, browser_use, computer_use, and MCP seams | Approved personal owner-mode Windows automation surface. Browser-use, Playwright/Playwright MCP, Windows-MCP, and local UIA fallback remain behind ToolWorker and CapabilityRuntime approval. |
| desktop agent worker | documented surface | services/desktop_agent plus packages.desktop_agent_runtime | Approved local-only perception worker. Reads focused-window content through Windows UI Automation adapters and consumes screenpipe recall through the existing MCP adapter path. Passive perception remains safe bounded projection by default; live owner-mode ToolWorker automation may persist raw automation artifacts under the RawAutomationCapture contract. |
| computer-use runtime | policy-controlled surface | services/tool_worker plus packages.adapters.capabilities and packages.automation_runtime | Approved personal owner-mode execution surface for Browser-use and Windows desktop computer-use actions after CapabilityRuntime policy and approval resume. Credential entry, arbitrary desktop control, raw screen/DOM/screenshot/keystroke/action artifact persistence, default Chrome profile attempts, Playwright MCP, and Windows-MCP are allowed in owner mode. Delete, shutdown, restart, registry, PowerShell, and comparable destructive actions require explicit per-action approval. |
| proactive runtime | policy-controlled surface | packages.proactive_runtime plus packages.learning_runtime | Approved bounded initiative-proposal runtime. Watches safe DesktopAgent projections and Assistant STATE, applies user-mutable learning preferences, stays explicit and visible, and never starts hidden background actions. |
...
| future desktop agent | future product surface | future desktop agent | Compatibility marker for governance gates: desktop vision, overlay behavior, credential extraction, and uncontrolled OS automation remain future/forbidden even though the local-only safe DesktopAgent perception worker is approved. |
...
| future proactive behavior | future product surface | future proactive worker | Compatibility marker for governance gates: hidden autonomous background actions remain future/forbidden even though visible local proposal-only proactive behavior is approved. |
