...
float noise2D(vec2 p) {
    ...
    return 0.5 + 0.5 * mix(mix(dot(hash2(i), f), dot(hash2(i + vec2(1,0)), f - vec2(1,0)), u.x), mix(dot(hash2(i + vec2(0,1)), f - vec2(0,1)), dot(hash2(i + vec2(1,1)), f - vec2(1,1)), u.x), u.y);
}

float sharpRing(vec3 d, float t) { float n = mix(noise2D(vec2(d.x, t) * 5.0), noise2D(vec2(d.y, t) * 5.0), d.z); return 1.0 + (n - 0.5) * 2.5 * 0.3 * 1.5; }
float smoothRing(vec3 d, float t) { float n = mix(noise2D(vec2(d.x, t) * 6.0), noise2D(vec2(d.y, t) * 6.0), d.z); return 0.9 + (n - 0.5) * 5.0 * 0.2; }
float flow(vec3 d, float t) { return mix(texture(uPerlinTexture, vec2(t, d.x / 2.0)).r, texture(uPerlinTexture, vec2(t, d.y / 2.0)).r, d.z); }
...
void main() {
    ...
    float ir1 = radius + uInputVolume * 0.2, ir2 = radius + uInputVolume * 0.15;
    float ra1 = (ir2 >= rr1) ? mix(0.2, 0.6, uInputVolume) : 0.0;
    float ra2 = smoothstep(rr2 - 0.05, rr2 + 0.05, ir1) * mix(0.15, 0.45, uInputVolume);
    float totalRingAlpha = max(ra1, ra2);
