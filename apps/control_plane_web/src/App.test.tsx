...
describe("Control Plane app", () => {
  ...
  it("renders marketplace, policy, memory, trace, and diagnostics views from safe endpoints", async () => {
    const responses: Record<string, unknown> = {
      ...
      "/control/voice": { schema_version: "1", summary: { main_stt_backend_id: "moonshine-v2", main_tts_backend_id: "supertonic-v2", wakeword_enabled: false, no_raw_audio_persistence_by_default: true }, settings: { wakeword: { phrase: "Hey Marvex", always_listening_enabled: false }, vad: { backend: { main_backend_id: "silero-vad" } }, barge_in: { enabled: true }, early_speech: { enabled: true }, personality: { active_voice_id: "M1" } }, backends: { backend_health: [{ backend_id: "moonshine-v2", package_name: "moonshine-voice", import_available: true }, { backend_id: "supertonic-v2", package_name: "supertonic", import_available: true }] }, telemetry: { wakeword_detections: 0, raw_audio_persisted: false, raw_transcript_persisted: false }, raw_audio_persisted: false, raw_transcript_persisted: false },
      "/control/voice/worker": { schema_version: "1", worker_id: "local-voice-worker", lifecycle_state: "stopped", process_started: false, heartbeat_ok: false, active_stt_backend_id: "moonshine-v2", active_tts_backend_id: "supertonic-v2", active_voice_id: "M1", active_tts_speed: 1.05, active_tts_quality_steps: 8, mic_status: "stopped", playback_status: "stopped", wakeword_status: "disabled", queued_tts_count: 0, local_only: true, hidden_recording_allowed: false, raw_audio_persisted: false, raw_transcript_persisted: false },
...
describe("Control Plane settings", () => {
  ...
  it("renders provider configuration in Settings and posts runtime config updates", async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText("Provider mode"), "litellm_proxy");
    expect(screen.getByLabelText("Provider mode")).toHaveValue("litellm_proxy");
    ...
    expect(JSON.parse(String(endpointCall?.[1]?.body))).toMatchObject({ provider_mode: "litellm_proxy" });
