...
from packages.adapters.capabilities.browser import BrowserActionKind, BrowserActionProposal, BrowserSessionRef
...
def run_end_to_end_assistant_turn(
    ...
    *,
    ...
    context_pack = _build_context(turn_input, intent.selected_intent, mcp_listings=mcp_listings)
    ...
    if intent.selected_intent.intent_kind == IntentKind.BROWSER_COMPUTER_USE:
        assistant_result, tool_projection, lifecycle_projection = _handle_browser_turn(turn_input, store, resume_approval_request_id)
    ...
    _emit(telemetry_sink, turn_input, TraceStage.TURN_COMPLETED, "Integrated assistant turn completed.", {"status": str(tool_projection.get("result_status", "success"))})
...
def _handle_browser_turn(turn_input: AssistantTurnInput, store: EndToEndTurnStateStore, resume_approval_request_id: str | None) -> tuple[AssistantTurnResult, dict[str, Any], dict[str, Any]]:
    proposal = _browser_proposal(turn_input)
    ...
    if approval_decision is not None and approval_decision.decision == "approved":
        ...
        tool_projection = {"approval_decision": "approved", "pending_approval_count": 0, "execution_request_present": True, "provider_continuation_ready": False, "final_response_ready": False, "result_status": result.status, "browser_action_count": 1, "raw_payload_persisted": False}
    ...
    tool_projection = {"pending_approval_count": 1, "provider_continuation_ready": False, "final_response_ready": False, "result_status": "requires_human_approval", "browser_action_count": 1, "raw_payload_persisted": False}
...
def _build_context(turn_input: AssistantTurnInput, intent_ref: Any, *, mcp_listings: tuple[McpToolListingProjection, ...] = ()) -> Any:
    ...
    if intent_ref.intent_kind == IntentKind.CAPABILITY_TOOL:
        ...
        candidates.append(ContextCandidate.from_safe_summary(ContextSourceRef(kind=ContextSourceKind.MEMORY_PROJECTION, identifier="memory.preference.short-answer"), "User prefers concise answers.", token_estimate=8, intent_tags=(IntentKind.MEMORY.value,)))
