...
class SQLiteMemoryTreeIndex:
    ...
    def safe_scores(self) -> tuple[dict[str, object], ...]:
        rows = self._rows("SELECT chunk_id, importance, decision FROM memory_tree_scores ORDER BY chunk_id", ())
        return tuple({"chunk_id": row[0], "importance": row[1], "decision": row[2], "raw_content_persisted": False} for row in rows)

    def safe_tree_nodes(self, *, tree_kind: str, tree_key: str) -> tuple[dict[str, object], ...]:
        rows = self._rows(
            "SELECT node_id, title, evidence_count FROM memory_tree_nodes WHERE tree_kind = ? AND tree_key = ? ORDER BY node_id",
            (tree_kind, tree_key),
        )
        return tuple({"node_id": row[0], "title": row[1], "evidence_count": row[2], "raw_content_persisted": False} for row in rows)
