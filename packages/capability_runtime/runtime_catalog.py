    ...
    return RuntimeCapabilityCatalog(
        capabilities=(
            ...
            RuntimeCapabilitySummary(
                ...
                source="adapter",
                intent_tags=("memory", "memory_tree_needed", "grounded_answer"),
                risk_level=ToolRiskLevel.SAFE,
                side_effect_level=ToolSideEffectLevel.READ_ONLY,
                prompt_summary="Agent-callable approved memory search with safe previews and refs.",
            ),
            RuntimeCapabilitySummary(
                identifier="memory.remember",
                display_name="Remember Memory",
                source="adapter",
                intent_tags=("memory", "provider_simple_chat"),
                risk_level=ToolRiskLevel.LOW,
                side_effect_level=ToolSideEffectLevel.WRITE_LOCAL,
                prompt_summary="Agent-callable explicit user memory write; non-explicit writes become candidates.",
            ),
            RuntimeCapabilitySummary(
                ...
                risk_level=ToolRiskLevel.SAFE,
                side_effect_level=ToolSideEffectLevel.WRITE_LOCAL,
                prompt_summary="Agent-callable exact-ref memory forget; broad deletion is not exposed as an auto tool.",
            ),
            RuntimeCapabilitySummary(
            ...
            RuntimeCapabilitySummary(
                ...
                side_effect_level=ToolSideEffectLevel.BROWSER_ACTION,
                approval_required=True,
                prompt_summary="Owner-mode Browser-use execution through ToolWorker with raw artifact persistence when approved.",
            ),
            RuntimeCapabilitySummary(
                identifier="playwright_mcp.task",
                display_name="Playwright MCP",
                source="adapter",
                intent_tags=("browser_computer_use", "mcp_needed"),
                risk_level=ToolRiskLevel.HIGH,
                side_effect_level=ToolSideEffectLevel.BROWSER_ACTION,
                approval_required=True,
                prompt_summary="Built-in local Playwright MCP backend for headed browser automation, extension mode, and deterministic browser tasks.",
            ),
            RuntimeCapabilitySummary(
                identifier="computer_use.action",
