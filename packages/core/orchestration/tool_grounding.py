...
def available_tools_grounding(*, now: datetime | None = None) -> str:
    ...
    return (
        ...
        "To act on the web or desktop, call a tool: use 'browser_use' for an "
        "autonomous multi-step web task, 'playwright_browser' for a specific "
        "browser step (navigate/click/type), or 'computer_use' for a Windows "
        "desktop action. These run only after the user approves; describe the "
        "task clearly and let the approval gate handle execution.\n"
