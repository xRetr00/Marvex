...
set "ShaOut=%~2"
set "ShaInclude=%~3"
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $root=(Resolve-Path -LiteralPath $env:ShaRoot).Path.TrimEnd('\'); $out=[IO.Path]::GetFullPath($env:ShaOut); $rootUri=New-Object Uri(($root+'\')); $files=New-Object 'System.Collections.Generic.List[System.IO.FileInfo]'; foreach($item in $env:ShaInclude.Split(';')){ $path=Join-Path $env:ShaRoot $item; if($item -match '[*?]'){ Get-ChildItem -Path $path -File -Recurse -ErrorAction SilentlyContinue | %% { $files.Add($_) } } elseif(Test-Path -LiteralPath $path -PathType Container){ Get-ChildItem -LiteralPath $path -File -Recurse -ErrorAction SilentlyContinue | %% { $files.Add($_) } } elseif(Test-Path -LiteralPath $path -PathType Leaf){ $files.Add((Get-Item -LiteralPath $path)) } }; $lines=$files | Where-Object { [IO.Path]::GetFullPath($_.FullName) -ne $out } | Sort-Object FullName -Unique | %% { $full=[IO.Path]::GetFullPath($_.FullName); $fileUri=New-Object Uri($full); $rel=[Uri]::UnescapeDataString($rootUri.MakeRelativeUri($fileUri).ToString()); $hash=(Get-FileHash -Algorithm SHA256 -LiteralPath $full).Hash.ToLowerInvariant(); \"$hash  $rel\" }; if(-not $lines){ throw \"No files matched for SHA manifest: $out\" }; Set-Content -LiteralPath $out -Value $lines -Encoding ASCII; Write-Host \"[OK] SHA256 manifest written: $out\""
if errorlevel 1 (
