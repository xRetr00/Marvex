...
`sounddevice==0.5.5` is declared as the local audio device dependency and isolated behind `SoundDeviceAudioAdapter`; tests use `FakeLocalAudioAdapter` because CI cannot validate physical microphone or speaker hardware. The worker can list/test fake devices, select devices through config reload, capture PCM frames without persistence, run a manual voice turn through VAD/STT/policy/assistant/TTS/playback event stages, emit safe event-count telemetry summaries, and interrupt playback for barge-in. Real model downloads and physical device smoke remain explicit local/manual work.
