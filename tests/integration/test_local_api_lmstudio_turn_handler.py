...
def test_lmstudio_handler_passes_fake_api_key_to_bridge_without_request_body(
    monkeypatch,
):
    import packages.runtime_composition.local_api_lmstudio_turns as lmstudio_turns
    ...
    assert result.error is None
    assert captured["kwargs"]["lmstudio_responses_api_key"] == fake_api_key
    assert captured["kwargs"]["provider_options"] == {}
    assert fake_api_key not in json.dumps(result.model_dump())
